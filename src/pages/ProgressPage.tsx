...
export const ProgressPage = () => {
  ...
  return (
    <div className="stack-lg">
          ...
          {last7.map(({ day, stat }) => {
            ...
            return (
              <div key={day} className="bar-row">
                ...
                <span className="bar-value">{stat.tasks} Aufgaben</span>
              </div>
            )
          })}
        </div>
      ...
      <section className="task-card">
        <h2>Häufige Fehlerarten</h2>
        {topErrors.length > 0 ? (
          <ul className="error-list">
            {topErrors.map((error) => (
              <li key={error.key}>
                <span>{labelForError(error.key)}</span>
                <strong>{error.count}</strong>
              </li>
            ))}
          </ul>
        ) : (
          <p>Noch keine Fehlerdaten vorhanden.</p>
        )}
      </section>
    </div>
  )
